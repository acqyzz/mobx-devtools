...
export const createChangeProcessor = (onChange: (change: Change) => void) => {
  let path = [];

  const push = (_change: PureSpyEvent, mobx) => {
    const change: Change = Object.create(null);
    ...
    if (isGroupEnd) {
      if (path.length === 0) {
        ...
        if (__isDEV__) frontendLogger.warn`groupStart & groupEnd mismatch`;
        return;
      }
